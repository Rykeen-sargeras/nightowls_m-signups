...
const API = {
    ...
    async signup(username, wowClass, specialization, eventType = "mythicplus", signupStatus = "available", canProvideLust = false, canInterrupt = false) {
        const res = await fetch(`${CONFIG.API_URL}/api/signup`, {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                username, wow_class: wowClass, specialization,
                event_type: eventType, signup_status: signupStatus,
                can_provide_lust: canProvideLust, can_interrupt: canInterrupt,
            }),
        });
    ...
    // --- GROUPS ---
    async getSortingSettings(password) {
        const res = await fetch(`${CONFIG.API_URL}/api/groups/sorting-settings/get`, {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ password }),
        });
        return await this._parseResponse(res, "Failed to load sorting settings");
    },
    async saveSortingSettings(password, blockedPairs) {
        const res = await fetch(`${CONFIG.API_URL}/api/groups/sorting-settings`, {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ password, blocked_pairs: blockedPairs }),
        });
        return await this._parseResponse(res, "Failed to save sorting settings");
    },
