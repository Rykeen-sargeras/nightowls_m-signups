const Admin = {
    ...
    renderPanel() {
        document.getElementById("adminInner").innerHTML = `
            ...
            <div id="adminLoggedIn" style="display:none;">
                ...
                <div class="admin-sort-settings">
                    <label for="blockedPairsInput">Blocked pairs</label>
                    <textarea id="blockedPairsInput" rows="4" placeholder="One pair per line: PlayerA | PlayerB"></textarea>
                    <button class="btn btn-sm btn-secondary" id="btnBlockedPairs" style="width:100%;margin-top:4px;">Save Blocked Pairs</button>
                    <small>These players will never be placed in the same M+ group.</small>
                </div>
            </div>
    ...
    _wireAdminButtons() {
        ...
        document.getElementById("btnBlockedPairs").addEventListener("click", () => this.saveSortingSettings());
    },
    ...
    async action(type) {
        ...
        try {
            ...
            if (type === "lock") {
                ...
                UI.renderGroups(result.groups, result.bench);
                UI.toast(result.message || "Locked & sorted!");
                this.log(result.message || `Formed ${result.groups.length} groups, ${result.bench.length} benched`);
            } else if (type === "unlock") {
                ...
                App.benchPriorityNames = new Set(bpData.bench_priority || []);
            }
    ...
    async loadSortingSettings() {
        const pw = this.getPassword();
        const input = document.getElementById("blockedPairsInput");
        if (!pw || !input) return;
        try {
            const result = await API.getSortingSettings(pw);
            input.value = (result.blocked_pairs || []).map(pair => `${pair[0]} | ${pair[1]}`).join("\n");
            this.log(`Loaded ${(result.blocked_pairs || []).length} blocked pair(s)`);
        } catch (err) {
            this.log("Could not load blocked pairs: " + err.message);
        }
    },

    _parseBlockedPairs(raw) {
        const pairs = [];
        const seen = new Set();
        String(raw || "").split(/\r?\n/).forEach(line => {
            ...
            if (parts.length !== 2) throw new Error(`Invalid blocked pair: ${trimmed}`);
            const first = parts[0].trim(), second = parts[1].trim();
            if (!first || !second || first.toLowerCase() === second.toLowerCase()) {
                throw new Error(`Invalid blocked pair: ${trimmed}`);
            }
            const key = [first.toLowerCase(), second.toLowerCase()].sort().join("|");
            if (!seen.has(key)) { seen.add(key); pairs.push([first, second]); }
        });
        return pairs;
    },

    async saveSortingSettings() {
        const pw = this.getPassword();
        const input = document.getElementById("blockedPairsInput");
        if (!pw || !input) return;
        try {
            const pairs = this._parseBlockedPairs(input.value);
            const result = await API.saveSortingSettings(pw, pairs);
            input.value = (result.blocked_pairs || []).map(pair => `${pair[0]} | ${pair[1]}`).join("\n");
            UI.toast(result.message || "Blocked pairs saved");
            this.log(result.message || "Blocked pairs saved");
        } catch (err) {
...
const DragDrop = {
    ...
    _updateBadges(div) {
        ...
        div.querySelectorAll(".player").forEach(p => {
            const utilityPlayer = { wow_class: p.dataset.cls, specialization: p.dataset.spec, can_provide_lust: p.dataset.canProvideLust === "true" };
            if (hasLust(utilityPlayer)) gl = true;
            if (hasBrez(utilityPlayer)) gb = true;
        });
