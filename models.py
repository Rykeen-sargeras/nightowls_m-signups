...
async def lifespan(app: FastAPI):
    async with engine.begin() as conn:
        ...
        await conn.execute(text("""
            ...
            ADD COLUMN IF NOT EXISTS can_provide_lust BOOLEAN DEFAULT FALSE,
            ADD COLUMN IF NOT EXISTS can_interrupt BOOLEAN DEFAULT FALSE,
            ADD COLUMN IF NOT EXISTS utility_overrides TEXT DEFAULT '{}'
        """))
        await conn.execute(text("""
            ...
            ADD COLUMN IF NOT EXISTS can_interrupt BOOLEAN DEFAULT FALSE,
            ADD COLUMN IF NOT EXISTS utility_overrides TEXT DEFAULT '{}',
            ADD COLUMN IF NOT EXISTS archive_batch VARCHAR(64) DEFAULT ''
        """))
        ...
        await conn.execute(text("UPDATE players SET can_provide_lust = FALSE WHERE can_provide_lust IS NULL"))
        await conn.execute(text("UPDATE players SET can_interrupt = FALSE WHERE can_interrupt IS NULL"))
        await conn.execute(text("UPDATE players SET utility_overrides = '{}' WHERE utility_overrides IS NULL"))
        await conn.execute(text("UPDATE archived_players SET can_provide_lust = FALSE WHERE can_provide_lust IS NULL"))
        await conn.execute(text("UPDATE archived_players SET can_interrupt = FALSE WHERE can_interrupt IS NULL"))
        await conn.execute(text("UPDATE archived_players SET utility_overrides = '{}' WHERE utility_overrides IS NULL"))
        await conn.execute(text("UPDATE archived_players SET archive_batch = '' WHERE archive_batch IS NULL"))
        await conn.execute(text("CREATE INDEX IF NOT EXISTS ix_archived_players_archive_batch ON archived_players (archive_batch)"))
...
app = FastAPI(title="NightOwls Mythic+ API", version="3.1.0", lifespan=lifespan)
